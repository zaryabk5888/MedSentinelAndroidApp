package com.example.myapplication.blockchainapp.presentation.users
...
@Composable
fun FailedAuth(navController: NavHostController) {
    ...
    val messagesCollection = db.collection(
      "Failed-Auth"
    ).document(FirebaseAuth.getInstance().currentUser?.email.toString()).collection("Failed")
                ...
                if (messagesList != null){
                    LazyColumn(
                        ...
                        items(messagesList!!.size()) { message ->
                            FailedAuthMessageItem(messagesList!!.documents[message].get("message").toString())
                        }
                    }
                }
            }
...
@Composable
fun FailedAuthMessageItem(message: String) {
    Card(
